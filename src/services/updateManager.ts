...
export class UpdateManager {
  ...
  async loadManifestFromUrl(url = '/data/update_manifest.json') {
    try {
      ...
      if (manifest.schemaVersion !== '2.0' || manifest.season !== '2026-27') {
        throw new Error('Manifest incompatibile')
      }
  ...
  applyLegacyPayload(payload: LegacyUpdatePayload) {
    if (!payload || !Array.isArray(payload.players) || typeof payload.generatedAt !== 'string') {
      throw new Error('Pacchetto dati non valido.')
    }

    createUserBackup()

    const previous: SourcePlayerData[] = structuredClone(this.dataManager.getSourcePlayers())
    const next = this.dataManager.mergeLegacyUpdates(
      payload.players,
      payload.sourceLabel,
      payload.generatedAt
    )
